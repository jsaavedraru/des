...
### Se agrega una matriz de penalidades para cumplir la siguiente definición de negocio:"Es 5 veces peor clasificar un cliente como bueno cuando es malo, que malo cuando es bueno"
```{r}
set.seed(2)
...
control_rpart <- rpart.control(cp = 0)
...
## Se hace "prune" al árbol para determinar el nivel óptimo de complejidad
```{r}
...
## Se define un threshold para determinar como se clasifican según la probabilidad de de ser clasificado como "bueno" o "malo". Al hacer esto se genera un trade-off entre sensibilidad y especificidad del modelo. Para el escenario propuesto se recomienda un threshold bajo, aunque signifique clasificar de forma incorrecta a las personas que no presentan un riesgo.
```{r}
